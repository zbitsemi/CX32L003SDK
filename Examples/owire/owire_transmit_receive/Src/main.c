  ...
  while (1)
	{
			...
			flag_owire_tx_rx_cplt = 0;
			HAL_OWIRE_TransmitReceive_IT(&gOwireHandler, txData, 1, rxData, 8);
			while(!flag_owire_tx_rx_cplt);
	#else
			HAL_OWIRE_TransmitReceive(&gOwireHandler, txData, 1, rxData, 8, 1000);
			HAL_Delay(1000);
	#endif
	}
}
