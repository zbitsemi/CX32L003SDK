...
int main(void)
{
  ...
  while (1)
	{
	#if (OWIRE_IT_ENABLED == 1)	
			flag_owire_tx_cplt = 0;
			HAL_OWIRE_Transmit_IT(&gOwireHandler, txData, 3);
			while(!flag_owire_tx_cplt);
			printf("OWIRE Transmit IT txData is: %d %d% d\n",txData[0],txData[1],txData[2]);	
			HAL_Delay(1000);
	#else
			HAL_OWIRE_Transmit(&gOwireHandler, txData, 3, 1000);
			printf("OWIRE Transmit txData is: %d %d% d\n",txData[0],txData[1],txData[2]);
			HAL_Delay(1000);
	#endif
...
void HAL_OWIRE_MspInit(OWIRE_HandleTypeDef *howire)
{
	...
	*/
	...
	GPIO_InitStruct.OpenDrain = GPIO_OPENDRAIN;//GPIO_PUSHPULL;	开漏输出
