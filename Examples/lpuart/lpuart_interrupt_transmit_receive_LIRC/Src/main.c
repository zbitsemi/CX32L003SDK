...
int main(void)
{
  ...
  SystemClock_Config();
  // LOWPOWER模式下 SystemClock_Config()必须开启 RCC_OSCILLATORTYPE_LIRC 时钟

  /* Peripheral clock enable */
  __HAL_RCC_LPUARTx_CLK_ENABLE();	
	
	sLPUartxHandle.Instance = LPUARTx;
	sLPUartxHandle.Init.ClkSel = LPUART_CLOCK_LIRC;// LOWPOWER模式下 时钟源必须选择 LPUART_CLOCK_LIRC
	sLPUartxHandle.Init.ClkDiv = LPUART_CLOCK_DIV1;//LOWPOWER模式下 只能通过改预分频可改变波特率 BAUDRATE = 38400/4*LPUART_CLOCK_DIV(预分频) 波特率最大9600
	sLPUartxHandle.Init.LowPowerMode = LPUART_MODE_LOWPOWER; //选择低功耗模式
	sLPUartxHandle.Init.BaudRate = BAUDRATE;	// LOWPOWER模式下 此设置位不可用 
	sLPUartxHandle.Init.BaudDouble = LPUART_BAUDDOUBLE_DISABLE;	// 双波特率禁用
	sLPUartxHandle.Init.WordLength = LPUART_WORDLENGTH_8B; // 数据长度
	sLPUartxHandle.Init.Parity = LPUART_PARITY_NONE; // 无校验位
	sLPUartxHandle.Init.Mode = LPUART_MODE_TX_RX; // 接收和发送使能 
	HAL_LPUART_Init(&sLPUartxHandle);
	
	HAL_LPUART_Transmit_IT(&sLPUartxHandle, ucSendData, sizeof(ucSendData));// 开启中断发送
	HAL_LPUART_Receive_IT(&sLPUartxHandle, ucReceiveData, sizeof(ucReceiveData));	// 开启中断接收
	HAL_NVIC_SetPriority(LPUARTx_IRQn,0); // 中断优先级
	HAL_NVIC_EnableIRQ(LPUARTx_IRQn); // 使能 LPUART 中断

  while (1)
	{
		/* LPUART receive LENGTH datas then transmit the received datas */		
		if(ucRxCompleteFlag == 1)
		{
			ucRxCompleteFlag = 0;
			HAL_LPUART_Transmit_IT(&sLPUartxHandle, ucReceiveData, sizeof(ucReceiveData));	
			HAL_LPUART_Receive_IT(&sLPUartxHandle, ucReceiveData, sizeof(ucReceiveData));				
		}		
	}
}
