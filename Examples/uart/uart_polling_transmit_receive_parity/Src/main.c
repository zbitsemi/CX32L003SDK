...
int main(void)
{
  ...
  __HAL_RCC_UARTx_CLK_ENABLE();	
	...
	sUartxHandle.Init.WordLength = UART_WORDLENGTH_9B; // 数据长度
	sUartxHandle.Init.Parity = UART_PARITY_ODD; // 奇校验位
	sUartxHandle.Init.Mode = UART_MODE_TX_RX; // 接收和发送使能
	HAL_UART_Init(&sUartxHandle);
	
	HAL_UART_Transmit(&sUartxHandle, ucSendData, sizeof(ucSendData), TIMEOUT_MAX);
	
	/* UART receive LENGTH datas then transmit the received datas */	
	HAL_UART_Receive(&sUartxHandle, ucReceiveData, sizeof(ucReceiveData), TIMEOUT_MAX);	
	HAL_UART_Transmit(&sUartxHandle, ucReceiveData, sizeof(ucReceiveData), TIMEOUT_MAX);	
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
    ...
    PA2     ------> UART0_TXD
    */
		GPIO_InitStruct.Pin = UARTx_RX_PIN; // GPIO选择
    GPIO_InitStruct.Mode = GPIO_MODE_AF; // 复用功能
		...
		GPIO_InitStruct.Alternate = UARTx_RX_ALTERNATE_AFn; // 端口复用
